...
import { Tag, Gift, Check, Sparkles, UserCheck } from 'lucide-react';
...
export const DiscountBenefitSection: React.FC<DiscountBenefitSectionProps> = ({
  ...
  // 짝꿍 코드/이름 입력 핸들러 (입력 시 자동 5만원 차감 활성화)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3.5">
        ...
        {/* 1-2. 짝꿍 할인 (빈칸 상시 노출 및 입력 시 자동 차감) */}
        <div
          className={`p-4 sm:p-5 rounded-2xl border transition-all ${
            partnerDiscount
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm sm:text-base font-semibold text-[#322A1B] break-keep flex items-center gap-1.5">
                <UserCheck className="w-4 h-4 text-[#8F7A56]" />
                <span>짝꿍 할인</span>
                {partnerDiscount && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-[#B09A74] text-white font-medium">
                    5만원 할인 적용 중
                  </span>
                )}
              </span>
          ...
          <div className="mt-3.5 pt-3.5 border-t border-[#F5F1EA] pl-7.5 space-y-1.5">
            ...
            <div className="relative">
              <input
                type="text"
                placeholder="예: 260927최한솔 (또는 추천인 성함 입력 시 자동 차감)"
                value={partnerName}
